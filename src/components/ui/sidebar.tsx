...
>(
  (
    ...
    const toggleSidebar = React.useCallback(() => {
      // The collapsible prop is available on the context, so we can check it here.
      // @ts-expect-error - collapsible is not in the context yet.
      if (isMobile || contextValue.collapsible === "offcanvas") {
...
>(
  (
    ...
    const context = React.useContext(SidebarContext)
    if (context) {
      // @ts-expect-error - collapsible is a custom prop.
    ...
    return (
      <Sheet open={isOpen} onOpenChange={handleOpen} {...props}>
        <SheetContent
...
>(
  (
    ...
    const content = href ? <Link href={href} className="flex gap-2 items-center">{children}</Link> : children;
    ...
    const button = (
      <Comp
        ...
        className={cn(sidebarMenuButtonVariants({ variant, size }), className, 'justify-start')}
        {...props}
      >
      {content}
      </Comp>
